<template>
    <div>
      <h2>Создать новое отделение</h2>
      <form @submit.prevent="createPostoffice">
        <div class="form-group">
          <label for="title">Номер:</label>
          <input v-model="postoffice.number" type="number" id="number" required>
        </div>
  
        <div class="form-group">
          <label for="address">Адрес:</label>
          <input v-model="postoffice.address" type="text" id="address" required>
        </div>
  ...
  export default {
    ...
    computed: {
      isValidForm() {
        return (
          this.postoffice.number &&
          this.postoffice.address
        );
      },
    },
    methods: {
      async createPostoffice() {
        try {
          const response = await axios.post('http://127.0.0.1:8000/api/postoffices/', this.postoffice);
          console.log('Server response:', response.data);
          this.$root.eventBus.$emit('updatePostoffice');
          this.$router.push({ name: 'PostOfficeList' });
        } catch (error) {
          console.error(error);
        }
      },
